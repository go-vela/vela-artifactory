...
| `http_client_retries` | number of times to retry failed http attempts | `false` | `N/A` | `PARAMETER_HTTP_CLIENT_RETRIES`<br>`ARTIFACTORY_HTTP_CLIENT_RETRIES` |
| `http_client_retry_wait` | amount of time to wait between failed http attempts | `false` | `N/A` | `PARAMETER_HTTP_CLIENT_RETRY_WAIT`<br>`ARTIFACTORY_HTTP_CLIENT_RETRY_WAIT` |
